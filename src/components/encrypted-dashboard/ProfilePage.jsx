...
const ProfilePage = () => {
        ...
        if (currentUser?.email) {
            const fetchDoc = async () => {
                ...
                if (!querySnapshot.empty) {
                    ...
                    setDocId(docSnap.id);
                    setForm({ ...docData, email: currentUser.email });

                    // Automatically patch existing user if trial fields are missing
                    if (docData.trialStartedAt === undefined) {
                        toast.info('Menginisialisasi periode uji coba notifikasi untuk akun Anda.');
                        await updateDoc(docSnap.ref, {
                            trialStartedAt: new Date(),
                            trialExpiredNotified: false
                        });
                        // No need to setForm here, it will be re-fetched or can be updated locally if needed
                    }
                } else {
                    // This is a new user, no profile exists yet.
                    setDocId('');
                    setForm({ 
                        firstName: '', 
                        lastName: '', 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!docId) {
                const docRef = await addDoc(collection(db, 'salesmen'), { 
                    ...dataToSave, 
                    email: currentUser.email,
                    trialStartedAt: new Date(), // Set trial start time
                    trialExpiredNotified: false // Initialize notification flag
                });
                setDocId(docRef.id);
            } else {
